...
class Ui_Database(QDialog):
    def __init__(self):
        ...
        # self.table.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.)
    ...
    def load_data(self):
        # self.data = [row.data() for row in self.table.selectedIndexes()];
        # try:
        for row in self.table.selectedIndexes():
            self.data.append(row.data());
        Ui_Database.accept(self);
        # except IndexError: print('Select something');
